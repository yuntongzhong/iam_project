...
public class JpaRegisteredClientRepositoryAdapter implements RegisteredClientRepository {
    ...
    public void save(RegisteredClient registeredClient) {
        ...
        entity.setAuthorizationCodeTtlMinutes(durationToMinutes(
                registeredClient.getTokenSettings().getAuthorizationCodeTimeToLive(),
                entity.getAuthorizationCodeTtlMinutes(),
                5
        ));
        entity.setAccessTokenTtlMinutes(durationToMinutes(
                registeredClient.getTokenSettings().getAccessTokenTimeToLive(),
                entity.getAccessTokenTtlMinutes(),
                30
        ));
        entity.setRefreshTokenTtlMinutes(durationToMinutes(
                registeredClient.getTokenSettings().getRefreshTokenTimeToLive(),
                entity.getRefreshTokenTtlMinutes(),
                480
        ));
        entity.setReuseRefreshTokens(registeredClient.getTokenSettings().isReuseRefreshTokens());
    ...
    public RegisteredClient toRegisteredClient(AppClient appClient) {
        ...
        builder.tokenSettings(TokenSettings.builder()
                .authorizationCodeTimeToLive(Duration.ofMinutes(defaultMinutes(appClient.getAuthorizationCodeTtlMinutes(), 5)))
                .accessTokenTimeToLive(Duration.ofMinutes(defaultMinutes(appClient.getAccessTokenTtlMinutes(), 30)))
                .refreshTokenTimeToLive(Duration.ofMinutes(defaultMinutes(appClient.getRefreshTokenTtlMinutes(), 480)))
                .reuseRefreshTokens(Boolean.TRUE.equals(appClient.getReuseRefreshTokens()))
                .build());
        return builder.build();
    }

    private int defaultMinutes(Integer value, int fallback) {
        return value != null && value > 0 ? value : fallback;
    }

    private int durationToMinutes(Duration value, Integer existing, int fallback) {
        if (value != null && !value.isNegative() && !value.isZero()) {
            return Math.max(1, Math.toIntExact(value.toMinutes()));
        }
        return defaultMinutes(existing, fallback);
    }
}
